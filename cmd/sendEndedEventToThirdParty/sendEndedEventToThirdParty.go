...
import (
	...
	"github.com/MorhafAlshibly/coanda/internal/sendEndedEventToThirdParty"
...
var (
	fs            = ff.NewFlagSet("sendEndedEventToThirdParty")
...
func main() {
	ctx := context.TODO()
	err := ff.Parse(fs, os.Args[1:], ff.WithEnvVarPrefix("SEND_ENDED_EVENT_TO_THIRD_PARTY"), ff.WithConfigFileFlag("config"), ff.WithConfigFileParser(ff.PlainParser))
	if err != nil {
		fmt.Printf("%s\n", ffhelp.Flags(fs))
	...
	db := model.New(dbConn)
	// Create the app
	app := sendEndedEventToThirdParty.NewApp(
		sendEndedEventToThirdParty.WithSql(dbConn),
		sendEndedEventToThirdParty.WithDatabase(db),
		sendEndedEventToThirdParty.WithThirdPartyUri(*thirdPartyUri),
		sendEndedEventToThirdParty.WithApiKeyHeader(*apiKeyHeader),
		sendEndedEventToThirdParty.WithApiKey(*apiKey),
		sendEndedEventToThirdParty.WithTopLimit(int32(*topLimit)),
		sendEndedEventToThirdParty.WithLimit(int32(*limit)),
	)
	if !*lambda {
