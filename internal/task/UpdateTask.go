...
func (c *UpdateTaskCommand) Execute(ctx context.Context) error {
	...
	if iErr != nil {
		c.Out = &api.UpdateTaskResponse{
			Success: false,
			Error:   conversion.Enum(*iErr, api.UpdateTaskResponse_Error_value, api.UpdateTaskResponse_NOT_FOUND),
		}
		return nil
	}
	if c.In.Data == nil {
	...
	result, err := c.service.database.UpdateTask(ctx, model.UpdateTaskParams{
		ID:   c.In.Task.Id,
		Type: c.In.Task.Type,
		Data: data,
	})
