...
import (
	...
	"github.com/MorhafAlshibly/coanda/internal/bff/item"
	"github.com/MorhafAlshibly/coanda/internal/bff/resolvers"
...
func Run() {
	...
	// Create the metrics
	metrics, err := metrics.NewPrometheusMetrics(prometheus.NewRegistry(), "item", 8081)
	if err != nil {
		log.Fatalf("failed to create metrics: %v", err)
	}
	// Create the item service
	itemService := item.NewService(itemStore, itemCache, metrics)
